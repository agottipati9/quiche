...
LlmSender::DebugState::DebugState(const LlmSender& sender)
    : mode(sender.mode_),
...
LlmSender::DebugState::DebugState(const DebugState& state) = default;

LlmSender::LlmSender(QuicTime now, const RttStats* rtt_stats,
...
void LlmSender::SetInitialCongestionWindowInPackets(
...
bool LlmSender::InSlowStart() const { return mode_ == STARTUP; }

void LlmSender::OnPacketSent(QuicTime sent_time, QuicByteCount bytes_in_flight,
...
void LlmSender::OnPacketNeutered(QuicPacketNumber packet_number) {
  sampler_.OnPacketNeutered(packet_number);
}

bool LlmSender::CanSend(QuicByteCount bytes_in_flight) {
  return bytes_in_flight < GetCongestionWindow();
}

QuicBandwidth LlmSender::PacingRate(QuicByteCount /*bytes_in_flight*/) const {
...
QuicBandwidth LlmSender::BandwidthEstimate() const {
  return max_bandwidth_.GetBest();
}

QuicByteCount LlmSender::GetCongestionWindow() const {
  if (mode_ == PROBE_RTT) {
...
QuicByteCount LlmSender::GetSlowStartThreshold() const { return 0; }

bool LlmSender::InRecovery() const {
  return recovery_state_ != NOT_IN_RECOVERY;
}

void LlmSender::SetFromConfig(const QuicConfig& config,
...
void LlmSender::ApplyConnectionOptions(
...
void LlmSender::AdjustNetworkParameters(const NetworkParams& params) {
...
void LlmSender::OnCongestionEvent(bool /*rtt_updated*/,
...
CongestionControlType LlmSender::GetCongestionControlType() const {
  return kLLM;
}

QuicTime::Delta LlmSender::GetMinRtt() const {
  if (!min_rtt_.IsZero()) {
    return min_rtt_;
  }
...
QuicByteCount LlmSender::GetTargetCongestionWindow(float gain) const {
...
QuicByteCount LlmSender::ProbeRttCongestionWindow() const {
  return min_congestion_window_;
}

void LlmSender::EnterStartupMode(QuicTime now) {
  if (stats_) {
...
void LlmSender::EnterProbeBandwidthMode(QuicTime now) {
  mode_ = PROBE_BW;
...
bool LlmSender::UpdateRoundTripCounter(QuicPacketNumber last_acked_packet) {
...
bool LlmSender::MaybeUpdateMinRtt(QuicTime now,
...
void LlmSender::UpdateGainCyclePhase(QuicTime now,
                                     QuicByteCount prior_in_flight,
                                     bool has_losses) {
...
void LlmSender::CheckIfFullBandwidthReached(
...
void LlmSender::MaybeExitStartupOrDrain(QuicTime now) {
...
void LlmSender::OnExitStartup(QuicTime now) {
...
bool LlmSender::ShouldExitStartupDueToLoss(
...
void LlmSender::MaybeEnterOrExitProbeRtt(QuicTime now, bool is_round_start,
                                         bool min_rtt_expired) {
...
void LlmSender::UpdateRecoveryState(QuicPacketNumber last_acked_packet,
...
void LlmSender::CalculatePacingRate(QuicByteCount bytes_lost) {
...
void LlmSender::CalculateCongestionWindow(QuicByteCount bytes_acked,
...
void LlmSender::CalculateRecoveryWindow(QuicByteCount bytes_acked,
...
std::string LlmSender::GetDebugState() const {
  std::ostringstream stream;
  stream << ExportDebugState();
  return stream.str();
}

void LlmSender::OnApplicationLimited(QuicByteCount bytes_in_flight) {
...
void LlmSender::PopulateConnectionStats(QuicConnectionStats* stats) const {
  stats->num_ack_aggregation_epochs = sampler_.num_ack_aggregation_epochs();
}

LlmSender::DebugState LlmSender::ExportDebugState() const {
  return DebugState(*this);
}

static std::string ModeToString(LlmSender::Mode mode) {
  switch (mode) {
    case LlmSender::STARTUP:
      return "STARTUP";
    case LlmSender::DRAIN:
      return "DRAIN";
    case LlmSender::PROBE_BW:
      return "PROBE_BW";
    case LlmSender::PROBE_RTT:
      return "PROBE_RTT";
  }
  return "???";
}

std::ostream& operator<<(std::ostream& os, const LlmSender::Mode& mode) {
  os << ModeToString(mode);
  return os;
}

std::ostream& operator<<(std::ostream& os, const LlmSender::DebugState& state) {
  ...
  if (state.mode == LlmSender::STARTUP) {
